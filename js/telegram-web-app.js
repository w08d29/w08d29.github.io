...
(function () {
        ...
        if (isIframe) {
            window.addEventListener('message', function (event) {
                ...
                if (dataParsed.eventType == 'set_custom_style') {
                    if (event.origin === 'httpsweb.telegram.org') {
    ...
    function urlAppendHashParams(url, addHash) {
         url looks like 'httpsgame.compathquery=1#hash'
         addHash looks like 'tgShareScoreUrl=' + encodeURIComponent('tgbshare_game_scorehash=very_long_hash123')

        var ind = url.indexOf('#');
        if (ind  0) {
             httpsgame.compath - httpsgame.compath#tgShareScoreUrl=etc
            return url + '#' + addHash;
        }
        var curHash = url.substr(ind + 1);
        if (curHash.indexOf('=') = 0  curHash.indexOf('') = 0) {
             httpsgame.com#hash=1 - httpsgame.com#hash=1&tgShareScoreUrl=etc
             httpsgame.com#pathquery - httpsgame.com#pathquery&tgShareScoreUrl=etc
            return url + '&' + addHash;
        }
         httpsgame.com#hash - httpsgame.com#hashtgShareScoreUrl=etc
        if (curHash.length  0) {
            return url + '' + addHash;
        }
         httpsgame.com# - httpsgame.com#tgShareScoreUrl=etc
        return url + addHash;
    }
    ...
    function postEvent(eventType, callback, eventData) {
        ...
        if (window.TelegramWebviewProxy !== undefined) {
            ...
            try {
                var trustedTarget = 'httpsweb.telegram.org';
    ...
    function openProtoUrl(url) {
        ...
        var useIframe = navigator.userAgent.match(iOSiPhone OSiPhoneiPodiPadi)
             true
             false;
        if (useIframe) {
            var iframeContEl =
...
(function () {
    ...
    function parseColorToHex(color) {
        ...
        if ((match = ^s#([0-9a-f]{6})s$i.exec(color))) {
            ...
            (match =
                ^srgba((d+),s(d+),s(d+)(,s(d+.{0,1}d)))s$.exec(
                    color,
                ))
        ) {
    ...
    WebApp.switchInlineQuery = function (query, choose_chat_types) {
        ...
        if (!initParams.tgWebAppBotInline) {
            console.error(
                '[Telegram.WebApp] Inline mode is disabled for this bot. Read more about inline mode httpscore.telegram.orgbotsinline',
            );
    ...
    WebApp.openTelegramLink = function (url) {
        ...
        if (isIframe  versionAtLeast('6.1')) {
            ...
            location.href = 'httpst.me' + path_full;
        }
    };
